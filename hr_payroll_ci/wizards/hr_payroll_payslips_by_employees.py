...
class HrPayslipEmployees(models.TransientModel):
    ...
    act_contract = fields.Boolean(string="Contrat en Cours")

    @api.onchange('act_contract')
    def onchange_contract(self):
        contract = self.env['hr.contract'].search([('state', '=','open')])
        contract_ids = contract.mapped('id')
        for rec in self:
            if rec.act_contract == True:
                rec.employee_ids = self.env['hr.employee'].search([('contract_id', 'in', contract_ids)])
